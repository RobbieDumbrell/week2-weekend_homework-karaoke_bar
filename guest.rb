...
class Guest
  ...
  def buy_food(food, bar)
    ...
    if bar.check_if_in_stock(food) == false
      return "Bar does not have that item in stock!"
    end
    ...
    if @wallet >= food.price  # if guest has enough money.
      ...
      bar.remove_stock_item(food) # removes one item form bar's stock.
      if @drunkeness < 0
        @drunkeness = 0 # prevents negative drunkeness
      end
    end
  ...
  def buy_drink(drink, bar)
    ...
    if bar.check_if_in_stock(drink) == false
      return "Bar does not have that item in stock!"
    end
